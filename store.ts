...
const defaults: Record<AppId, Omit<WebtopWindow, 'id' | 'appId' | 'zIndex' | 'minimized' | 'maximized' | 'fullscreen' | 'spaceId'>> = {
...
export const useWebtop = create<WebtopState>((set, get) => ({
  ...
  minimizeWindow: id =>
    set(s => ({
      windows: s.windows.map(w => (w.id === id ? { ...w, minimized: true, fullscreen: false } : w)),
      focusedId: s.focusedId === id ? undefined : s.focusedId,
    })),
  ...
  snapWindow: (id, edge) => {
    ...
    const screenH = window.innerHeight - 78; // menu + dock space
  ...
  removeSpace: id => {
    ...
    // Move windows from deleted space to the new active one
    set({
      spaces: remaining,
