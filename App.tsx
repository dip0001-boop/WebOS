...
function WindowFrame({ win, userId, children }: { win: WebtopWindow; userId: string; children: ReactNode }) {
  ...
  return (
    <Rnd
      ...
      onDragStop={(_, d) => { const patch = { x: d.x, y: d.y, snapped: null as const }; updateWindow(win.id, patch); persist({ ...win, ...patch }); }}
      onResizeStop={(_, __, ref, ___, pos) => {
        const patch = { width: ref.offsetWidth, height: ref.offsetHeight, x: pos.x, y: pos.y, snapped: null as const };
...
function Notes({ userId, nodeId, full = false }: { userId: string; nodeId?: string; full?: boolean }) {
  ...
  return (
    <div className={full ? 'notes full' : 'notes'}>
      ...
      <textarea value={note?.content || ''} placeholder="Select a note" onChange={e => {
        if (!note) return;
...
function SearchApp() {
  ...
  async function doSearch(e?: FormEvent) {
    ...
    setSearched(true);
    try {
      const res = await fetch(`https://searx.be/search?q=${encodeURIComponent(query)}&format=json&categories=general`);
    ...
    <div className="search-app">
      <div className="search-hero">
        ...
        <form onSubmit={doSearch} className="search-form">
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search the web..." autoFocus />
        ...
        <div className="search-results">
          ...
          {results.map((r, i) => (
            <a key={i} href={r.url} target="_blank" rel="noreferrer" className="search-result">
              <div className="result-url">{r.pretty_url || r.url}</div>
...
export default function App() {
  ...
  const { windows, focusedId, preferences, activeSpaceId, setPreferences, restoreWindows, setMissionControl, setLaunchpad, toggleFullscreen } = useWebtop();
  ...
  return (
    <div className={`desktop ${preferences?.theme || 'light'}`} style={{ backgroundImage: `url(${currentWallpaper.url})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
      <div className="menubar">
        <b>{focused?.title || 'Webtop'}</b>
        <span>
          <User size={14} /> {clock.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
          <button className="logout" onClick={() => supabase?.auth.signOut().finally(() => setUserId(undefined))}><LogOut size={14} /></button>
        </span>
      </div>
